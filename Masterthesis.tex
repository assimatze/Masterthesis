...
\begin{description}
   ...
   \begin{enumerate}
      ...
      \item Was gefällt euch, wenn ihr in Confluence einen Eintrag ließt?
      \item Was gefällt euch \textbf{NICHT}, wenn ihr in Confluence einen Eintrag ließt?
...
\\\\
\textbf{Fragen 1,2,3 der Schreib-Prozess}\\
Aus der Umfrage ergaben sich folgende Ergebnisse für den Schreib-Prozess in Confluence. Es wurde von den Player als positiv gewertet, dass das Erstellen eines neuen Eintrages in die Confluence Wiki einfach und schnell funktioniert. Man kann immer über einen einzigen Klick an Ort und Stelle einen neuen Eintrag generieren (Abb.\ref{ConfluenceEintragErstellen}). Ein weiterer Vorteil der Confluence Wiki die von den Playern bestätigt wurde ist, dass die Einträge sichtbar für alle Mitarbeiter sind und es den Player ermöglicht wird sofort nach der Erstellung des Eintrages darauf zuzugreifen. Außerdem wurden die verschiedenen Struktur Möglichkeiten in Form von Tabellen oder Inhaltsverzeichnisse ebenfalls als positiv gewertet. Dadurch lassen sich Einträge in der Anwendung unterschiedlich anordnen und man kann ihnen sonder Funktionen zuweisen wie z.B das bestätigen einer Checkliste die als eine einfache Tabelle umgesetzt wird. 
\begin{figure}[h!]
\begin{center}
...
\end{figure} 
\\\\
Beschwert haben sich die Player über einen eingeschränkten Editor und über die Anordnung der Editor-Funktionen. Des weiteren gaben die Player im Fragebogen an, dass die Confluence Wiki unübersichtlich sei und die Features nicht richtig genutzt werden. Die Optik und das Layout der Seite wurden ebenfalls als negative Eigenschaften gewertet. Worüber sich die Player weiterhin beschwert haben war, dass die Einträge die gemacht werden schwierig einzuordnen sind. Weiß ein Player nicht genau, wo der Eintrag beim erstellen eingeordnet werden soll, so kann das Verschieben dieses Eintrags später für Schwierigkeiten sorgen.
\\\\
Die Verbesserungen die von den Playern vorgeschlagen wurden beinhalten unter anderem, dass der Editor über mehr bzw. bessere Features erweitert wird und das diese auch sinnvoll gruppiert werden. Hinzukommen sollen außerdem noch mehr Freiheiten für die Gestaltung und Strukturierung des Eintrages. Es wird gewünscht das Layout zu verbessern und das beim Schreiben intelligente Vorschläge gemacht werden wo der Eintrag einzuordnen ist.
\\\\
\textbf{Fragen 4,5,6 der Lese-Prozess}\\
Die Antworten auf die Fragen zum Lese-Prozess ergaben, dass die Player zwischen den Aktivitäten des Lesens von Einträgen und Suchen von Einträgen keine Unterscheidung treffen. So hat die Frage \textit{Was gefällt euch, wenn ihr in Confluence einen Eintrag ließt?} gezeigt, dass der Lese-Prozess nur dann als positiv empfunden wird, wenn die Einträge die bei der Suche zurückgeliefert werden auch die Antworten auf spezifische Problemstellungen des Players enthält. Hierfür ist ein strukturierter Überblick hilfreich und kurzgehaltene gut strukturierte Inhalte. Dadurch kann der Player effektiv bestimmen, ob der Eintrag für ihn relevant ist. 
\\\\
Bei der Umfrage gaben die Player auf die Frage \textit{Was gefällt euch \textbf{NICHT}, wenn ihr in Confluence einen Eintrag ließt?} folgende Antworten. Die Player führten als negativen Aspekt auf, dass es erforderlich ist den Autor bei unverständlichen Einträgen direkt anzusprechen. Die Volltextsuche von Confluence wurde ebenfalls bemängelt. Die Volltextsuche liefert dem Player als Ergebnis auf seine Anfrage zu viele Einträge, die alle nach dem passenden Inhalt durchsucht werden müssen. Der Letzte Kritikpunkt der von den Playern aufgeführt wurde betrifft die Aktualität mancher Einträge. Wie wir bereits in Kapitel \ref{Zeitersparnis} ermittelt haben, ist Aktualität ein wichtiges Kriterium des Knowledge Managements. Dass das Sicherstellen von Aktualität aber ein ernstes Problem für die Player darstellt wird durch diese Umfrage nochmal deutlich.
\\\\
...
\\\\
\textbf{Fragen 7,8,9 der Such-Prozess}\\
Obwohl der Such-Prozess bereits im vorherigen Abschnitt erwähnt und zum Teil auch analysiert wurde, werden die positiven Eigenschaften der Suche welche dem Player wichtig sind hier noch ausführlicher aufgeführt. Die Suche in der Confluence Anwendung bietet beim eingeben der Anfrage bereits Stichworte(Abb.\ref{Sucheingabe}). Dieses Feature ist zwar nützlich, funktioniert nach angaben der Player aber nur wenn man weiß wonach man sucht. Dies liegt oft auch daran, dass die Einträge falsch oder mehrfach benannt sind.  
\\
\begin{figure}[h!]
\begin{center}
...
\end{figure}
\\
Wenn die gewöhnliche Such-Funktion von Confluence nicht ausreicht, verwenden die Player die Volltextsuch-Funktion. Diese liefert zwar zu viele Beiträge ist aber dafür auch für die Player einfach zu bedienen.  
\\\\
Als negativ wurde von den Player bei der Such-Funktion bewertet, dass sie wie gesagt zu viele Ergebnisse liefert und man bereits vor der Suche eigentlich wissen muss, wo genau die Inhalte hinterlegt wurden. Als eine weitere negative Eigenschaft der Suche wurde von den Playern vermerkt, dass Inhalte zu Themen nicht gefunden werden weil die entsprechenden Suchbegriffe nicht in die Anfrage mit aufgenommen wurden. Diese Synonyme stellen für die Player immer wieder ein Problem dar. 
\\\\
Die Such-Funktion die bei Confluence zum Einsatz kommt verfügt nach Ansicht der Player über klare Mängel. Als solches wünschen die Player sich eine Verbesserung dieser Funktion. Dabei wäre es für die Player schon ausreichend wenn die Such-Funktion auf Suchbegriffe toleranter reagieren würde. Dadurch könnten Einträge besser gefunden werden. Auch Vorgeschlagen wurde, den Suchalgorithmus zu verbessern. Zum Schluss kam die Idee die semantische Beziehungen der Suchbegriffe herzustellen bzw. inhaltliche Bezüge bei Suchergebnissen darzustellen.
