...
Durch die Design Methode des Player-Centirc Designs die wir in Kapitel \ref{Player-Centric Methode} beschrieben haben wissen wir, dass die Anwender beim Entwurf einer gamifizierten Lösung im Mittelpunkt stehen. Somit ist es also erforderlich sich mit dem Anwender, der in dieser Entwicklungsphase als Player bezeichnet werden soll, auseinander zu setzen und zu erfahren, was denn die Ziele des Players (Player Goals) sind. Neben den Zielen der Anwender ist es ebenfalls erforderlich, die Geschäftsziele (Business Objectives) für die Modifikationen an der Confluence Wiki zu definieren. Dies ist ein besonders wichtiger Punkt da Gamification ein neuer Trend ist, dem sich viele Firmen zu nutzen machen wollen. Dabei ist es ihnen egal wie die Anwendung oder die Geschäftsziele aussehen, Hauptsache es wird Gamification verwendet \cite{gamificationDefinition}. Jedoch kann man erst erfolgreich sein, wenn man definiert hat, wie Erfolg aussieht. Als solches müssen die Geschäftsziele in Metriken oder Kennzahlen erfasst werden und dies so spezifisch wie möglich \cite{gamificationDefinition}.
\\\\
Der Wissenserfassungsprozess kann in vier kleinere Handlungen des Anwenders aufgeteilt werden. Diese wären \textit{schreiben}, \textit{lesen}, \textit{suchen} und \textit{kommentieren}. Diese Handlungen stellen die Interaktion des Players mit der Confluence Wiki dar und eignen sich daher ideal für die Ermittlung der Player Goals. Zusätzlich können die Handlungen des Anwenders zum Erfassen von Kennzahlen und zum beschreiben der Geschäftsziele genutzt werden. Es kann also ermittelt werden, wie viele Anwender zu einem bestimmten Zeitpunkt Informationen in die Confluence Wiki \textit{geschrieben} haben oder wie viele Einträge bereits \textit{gelesen} bzw. aufgerufen wurden.  
\\\\
Zunächst muss die Zielgruppe bestimmt werden, welche als Player für die gamifizierte Confluence Wiki in frage kommen. Bei Gamification unterscheidet man in der Regel zwischen drei verschiedenen Zielgruppen für die die Anwendung gamifiziert wird. Die Zielgruppen teilen sich dabei in \textit{Kunden}, \textit{Mitarbeiter} und \textit{Communities} auf \cite{gamificationDefinition}. In unserem Fall sind die \textit{Mitarbeiter} die Player unserer gamifizierten Confluence Wiki. Über die Player sind jetzt auch weitere Informationen bekannt. So liegt das Alter der Player zwischen 25-55 Jahren wobei ca. 50\% der Mitarbeiter um die 30 Jahre alt ist. Von den Playern sind ca. 80\% männlich und haben entweder einen Studien Abschluss im Bereich Informatik oder BWL und sind entweder als Consultant oder Senior Consultant angestellt.
\\\\
Aus den vorliegenden Informationen können bereits wertvolle Erkenntnisse gewonnen werden. Circa 50\% der Mitarbeiter sind um die 30 Jahre alt und sind somit zu einer großen Wahrscheinlichkeit auch mit Videospielen aufgewachsen. Dadurch kann man vermuten, dass dieser Teil der Player besonders positiv auf die Spielmechaniken reagieren wird \cite{Persona2018}. Ein weiteres markantes Merkmal ist der große Anteil an männlichen Mitarbeitern. Männer sind in der Regel eher Wettkampf bezogen während Frauen z.B mehr auf soziale Werte ansprechen. Also kann man bei einer Anwendung die hauptsächlich von Männern genutzt erfolge erzielen in dem man Wettkampf-Bedingungen schafft.   
